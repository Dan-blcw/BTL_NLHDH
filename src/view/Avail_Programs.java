...
public class Avail_Programs extends JPanel implements ActionListener{
//	Point point ;
	Integer xInteger;
	private Graphics2D g2;
	public Avail_Programs() {
		initComponents();
        setOpaque(false);
	}
...
//    	if(oj != null) {
    		for (int i = 0; i < oj.size(); i++) {
            	...
            	SampleView.Avail_Programs.get(i).setCloColor(SampleView.Clo.get(i));
            	g2.setColor(SampleView.Clo.get(i));
            	g2.fillRect(oj.get(i).getX(), oj.get(i).getY(), 142, idt);
            	
            	g2.setColor(Color.BLACK);
                g2.drawString(oj.get(i).getIndexID(),40, oj.get(i).getY()+ (idt/2));
            }
//    	}
    }
	public void addg(Graphics g) {
        ...
        GradientPaint gp = new GradientPaint(0, 0, Color.decode("#e3eeff"), 0, getHeight(), Color.decode("#50c9c3"));
        g2.setPaint(gp);
...
//        	g2.fillRect(idt, idt, idt, idt);
//        }
        ArrayList<SampleModel> oj = SampleView.Avail_Programs;
        drawEllipses(g2,oj);
		g2.dispose();
	}
	public void remake() {
